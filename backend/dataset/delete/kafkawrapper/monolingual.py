...
from configs.configs import kafka_bootstrap_server_host, monolingual_input_topic, ulca_dataset_publish_consumer_grp
...
def consume():
    try:
        topics = [monolingual_input_topic]
        consumer = instantiate(topics)
        service = ParallelService()
        rand_str = ''.join(random.choice(string.ascii_letters) for i in range(4))
        prefix = "Monolingual-" + "(" + rand_str + ")"
        ...
        while True:
            for msg in consumer:
                try:
                    ...
                    log.exception(f'{prefix} Exception in monolingual consumer while consuming: {str(e)}', e)
    except Exception as e:
        log.exception(f'Exception in monolingual consumer while consuming: {str(e)}', e)
