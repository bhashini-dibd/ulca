...
class TransliterationValidate:
    def __init__(self):
        pass

    def execute_validation_pipeline(self, request):
        try:
            log.info("Executing transliteration dataset validation....  {}".format(datetime.now()))
            v_pipeline = ValidationPipeline.getInstance()
            res = v_pipeline.runTransliterationValidators(request)
            if res:
            ...
            log.exception(f"Exception in validation of transliteration dataset: {str(e)}", e)
            tracker_data = {"status": "FAILED"}
            pt.update_task_details(tracker_data)
