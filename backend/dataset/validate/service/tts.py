...
class TTSValidate:
    ...
    def execute_validation_pipeline(self, request):
        try:
            log.info("Executing TTS dataset validation....  {}".format(datetime.now()))
            duration = 0

            v_pipeline = ValidationPipeline.getInstance()
            res = v_pipeline.runTtsValidators(request)
            ...
            if res:
                ...
                if res["status"] == "SUCCESS":
                    unique_hash = request["record"]["textHash"] + request["record"]["audioHash"]
            ...
            log.exception(f"Exception in validation of tts dataset: {str(e)}", e)
            tracker_data = {"status": "FAILED", "duration": duration}
            pt.update_task_details(tracker_data)
