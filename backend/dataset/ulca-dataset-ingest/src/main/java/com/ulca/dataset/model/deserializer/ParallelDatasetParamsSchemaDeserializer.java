	...
	public ParallelDatasetParamsSchema deserialize(JsonParser p, DeserializationContext ctxt)
		...
		if (node.has("version")) {
			if (!node.get("version").isTextual()) {
				...
				parallelParamsSchema.setVersion(version);
			}
		...
		if (node.has("licenseUrl")) {
			if (!node.get("licenseUrl").isTextual()) {
				...
				String licenseUrl = node.get("licenseUrl").asText();
				parallelParamsSchema.setLicenseUrl(licenseUrl);
			}
						...
						if (node.get("collectionMethod").has("collectionDetails")) {
							...
							case AUTO_ALIGNED:
								if (node.get("collectionMethod").get("collectionDetails").has("alignmentTool")) {
									...
									if (CollectionDetailsAutoAligned.AlignmentToolEnum
										...
										parallelDatasetCollectionMethod
												.setCollectionDetails(collectionDetailsAutoAligned);
										parallelParamsSchema.setCollectionMethod(parallelDatasetCollectionMethod);
									}
								} else {
							...
							case AUTO_ALIGNED_FROM_PARALLEL_DOCS:
								if (node.get("collectionMethod").get("collectionDetails").has("alignmentTool")) {
									String alignmentTool = node.get("collectionMethod").get("collectionDetails")
											.get("alignmentTool").asText();
									if (CollectionDetailsAutoAlignedFromParallelDocs.AlignmentToolEnum
											.fromValue(alignmentTool) == null) {
										errorList.add("alignmentTool is not one of defined alignmentTool");
									} else {
										CollectionDetailsAutoAlignedFromParallelDocs collectionDetailsAutoAlignedFromParallelDocs = mapper
												.readValue(
														node.get("collectionMethod").get("collectionDetails")
																.toPrettyString(),
														CollectionDetailsAutoAlignedFromParallelDocs.class);

										parallelDatasetCollectionMethod
												.setCollectionDetails(collectionDetailsAutoAlignedFromParallelDocs);
										parallelParamsSchema.setCollectionMethod(parallelDatasetCollectionMethod);
									}
								} else {
									errorList.add("alignmentTool key has to be present");
							...
							case MACHINE_TRANSLATED:
								...
								CollectionDetailsMachineTranslated collectionDetailsMachineTranslated = mapper
										.readValue(
												node.get("collectionMethod").get("collectionDetails").toPrettyString(),
												CollectionDetailsMachineTranslated.class);
								parallelDatasetCollectionMethod
										.setCollectionDetails(collectionDetailsMachineTranslated);
								parallelParamsSchema.setCollectionMethod(parallelDatasetCollectionMethod);
								break;
