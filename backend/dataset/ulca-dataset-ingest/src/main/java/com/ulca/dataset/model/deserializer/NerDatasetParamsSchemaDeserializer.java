...
public class NerDatasetParamsSchemaDeserializer extends StdDeserializer<NerDatasetParamsSchema> {
	...
	public NerDatasetParamsSchema deserialize(JsonParser p, DeserializationContext ctxt)
		...
		} else if (!node.get("tagsFormat").isTextual()) {
		...
		if (node.has("isStopwordsRemoved")) {
			if (!node.get("isStopwordsRemoved").isBoolean()) {
				errorList.add("isStopwordsRemoved field should be boolean");
			} else {
				boolean isStopwordsRemoved = node.get("isStopwordsRemoved").asBoolean();
				nerParamsSchema.isStopwordsRemoved(isStopwordsRemoved);
			}
		...
		if (node.has("formatDescription")) {
			if (!node.get("formatDescription").isTextual()) {
				errorList.add("formatDescription field should be String");
			} else {
				String formatDescription = node.get("formatDescription").asText();
				nerParamsSchema.setFormatDescription(formatDescription);
			}
