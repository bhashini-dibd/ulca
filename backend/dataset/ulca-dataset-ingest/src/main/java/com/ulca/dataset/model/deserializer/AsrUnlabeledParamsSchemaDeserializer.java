...
public class AsrUnlabeledParamsSchemaDeserializer extends StdDeserializer<AsrUnlabeledParamsSchema> {
	...
	public AsrUnlabeledParamsSchema deserialize(JsonParser p, DeserializationContext ctxt)
		...
		/*
		 * ASR unlabeled Params schema is same as ASR Params schema
		 */
		for (String k : keys) {
			try {
						...
						if (node.get("collectionMethod").has("collectionDetails")) {
									...
									if (!node.get("collectionMethod").get("collectionDetails").get("alignmentTool")
										...
										if (alignmentToolEnum != null) {
											...
											collectionMethodAudio
													.setCollectionDetails(collectionDetailsAudioAutoAligned);
											asrParamsSchema.setCollectionMethod(collectionMethodAudio);
							...
							case MACHINE_GENERATED_TRANSCRIPT:
								...
								if (node.get("collectionMethod").get("collectionDetails").has("asrModel")) {
									collectionDetailsMachineGeneratedTranscript.setAsrModel(node.get("collectionMethod")
											.get("collectionDetails").get("asrModel").asText());
									collectionMethodAudio
											.setCollectionDetails(collectionDetailsMachineGeneratedTranscript);
								}
							...
							case MANUAL_TRANSCRIBED:
								...
								CollectionDetailsManualTranscribed collectionDetailsManualTranscribed = mapper
										.readValue(
