 ...
 * 
 * import com.ulca.benchmark.dao.BenchmarkDao; import
 * com.ulca.benchmark.dao.BenchmarkProcessDao; import
 * com.ulca.benchmark.kafka.model.BenchmarkIngest; import
 * com.ulca.benchmark.kafka.model.BmDatasetDownload; import
 * com.ulca.benchmark.model.BenchmarkProcess; import
 * com.ulca.benchmark.request.*; import com.ulca.benchmark.response.*; import
 * com.ulca.benchmark.util.ModelConstants; import com.ulca.model.dao.ModelDao;
 * import com.ulca.model.dao.ModelExtended; import
 * com.ulca.model.response.BmProcessListByProcessIdResponse; import
 * io.swagger.model.Benchmark; import io.swagger.model.LanguagePair; import
 * io.swagger.model.LanguagePairs; import io.swagger.model.ModelTask; import
 * io.swagger.model.SupportedLanguages; import io.swagger.model.SupportedTasks;
 * 
 ...
 * java.util.Optional; import java.util.stream.Stream;
 * 
 ...
 * 
 * @SpringBootTest class BenchmarkServiceTest {
 * 
 * @InjectMocks BenchmarkService benchmarkService;
 * 
 * @Mock BenchmarkDao benchmarkDao;
 * 
 * @Mock BenchmarkProcessDao benchmarkProcessDao;
 * 
 * @Mock ModelDao modelDao;
 * 
 * @Mock BenchmarkSubmtStatusService bmSubmitStatusService;
 * 
 * @Mock KafkaTemplate<String, BenchmarkIngest> benchmarkIngestKafkaTemplate;
 * 
 * @Mock KafkaTemplate<String, BmDatasetDownload>
 * benchmarkDownloadKafkaTemplate;
 * 
 * @Value("${kafka.ulca.bm.filedownload.ip.topic}") String
 * benchmarkDownloadTopic;
 * 
 * @BeforeEach void setUp() { }
 * 
 * @Test void submitBenchmark() { BenchmarkSubmitRequest request = new
 * BenchmarkSubmitRequest("test","test","test");
 * 
 * when(benchmarkDao.findByName("test")).thenReturn(null);
 * assertInstanceOf(BenchmarkSubmitResponse.class,benchmarkService.
 * submitBenchmark(request)); }
 * 
 * @Test void executeBenchmark() { ExecuteBenchmarkRequest request = new
 * ExecuteBenchmarkRequest(); request.setModelId("test"); Benchmark benchmark =
 * new Benchmark(); benchmark.setBenchmarkId("test");
 ...
 * when(benchmarkProcessDao.findByModelIdAndBenchmarkDatasetIdAndMetric("test",
 * "test","test")) .thenReturn(Collections.singletonList(benchmarkProcess));
 * 
 * assertInstanceOf(ExecuteBenchmarkResponse.class,benchmarkService.
 * executeBenchmark(request));
 * 
 * 
 * }
 * 
 * @Test void executeBenchmarkAllMetric() { ExecuteBenchmarkAllMetricRequest
 * request = new ExecuteBenchmarkAllMetricRequest(); request.setModelId("test");
 ...
 * when(benchmarkDao.findByBenchmarkId("test")).thenReturn(benchmark);
 * when(benchmarkProcessDao.findByModelIdAndBenchmarkDatasetIdAndMetric("test",
 * "test",modelConstants.getMetricListByModelTask("translation").get(0)))
 * .thenReturn(Collections.singletonList(benchmarkProcess));
 * 
 * assertInstanceOf(ExecuteBenchmarkResponse.class,benchmarkService.
 * executeBenchmarkAllMetric(request));
 * 
 * 
 * }
 * 
 ...
 * request.setTask("translation"); request.setSourceLanguage("en");
 * request.setTargetLanguage("hi");
 * 
 * return Stream.of(Arguments.of(request,1,1), Arguments.of(request,null,null));
 * }
 * 
 * @ParameterizedTest
 * 
 * @MethodSource("searchBenchmarkParam") void
 * searchBenchmark(BenchmarkSearchRequest request, Integer startPage, Integer
 * endPage) { ModelTask modelTask = null; LanguagePair lp = null;
 * 
 * if (request.getTask() != null && !request.getTask().isBlank()) { modelTask =
 * new ModelTask();
 ...
 * 
 * 
 * if(lp!=null && modelTask!=null) { if (startPage != null) {
 * when(benchmarkDao.findByTaskAndLanguages(modelTask, lp,
 * paging)).thenReturn(Collections.singletonList(new Benchmark())); } else
 * when(benchmarkDao.findByTaskAndLanguages(modelTask,
 * lp)).thenReturn(Collections.singletonList(new Benchmark())); }
 * assertEquals(new BenchmarkSearchResponse("Benchmark Search Result",
 * Collections.singletonList(new
 * Benchmark()),1),benchmarkService.searchBenchmark(request,startPage,endPage));
 * 
 * }
 * 
 * @Test void processStatus() {
 * 
 * List<BenchmarkProcess> list = Collections.singletonList(new
 * BenchmarkProcess());
 * 
 * when(benchmarkProcessDao.findByBenchmarkProcessId("test")).thenReturn(new
 * BenchmarkProcess()); assertEquals(new
 * BmProcessListByProcessIdResponse("Benchmark Process list", list,
 * list.size()),benchmarkService.processStatus("test")); }
 * 
 * @Test void getBenchmarkById() {
 * 
 * Benchmark benchmark = new Benchmark(); ModelTask modelTask = new ModelTask();
 * modelTask.setType(SupportedTasks.TRANSLATION); benchmark.setTask(modelTask);
 * 
 * 
 * BenchmarkProcess benchmarkProcess = new BenchmarkProcess(); ModelConstants
 * modelConstants = new ModelConstants();
 * ReflectionTestUtils.setField(benchmarkService,"modelConstants",modelConstants
 * ); benchmarkProcess.setMetric(modelConstants.getMetricListByModelTask(
 * "translation").toString()); benchmarkProcess.setStatus("Completed");
 * benchmarkProcess.setModelId("test");
 * 
 * ModelExtended modelExtended = new ModelExtended();
 * modelExtended.setName("test"); modelExtended.setStatus("unpublished");
 ...
 * 
 * when(benchmarkDao.findByBenchmarkId("test")).thenReturn(benchmark);
 * when(benchmarkProcessDao.findByBenchmarkDatasetId("test")).thenReturn(
 * Collections.singletonList(benchmarkProcess));
 * 
 ...
 * @ParameterizedTest
 * 
 * @MethodSource("benchmarkListByUserIdParam") void benchmarkListByUserId(String
 * userId, Integer startPage, Integer endPage,Integer pgSize,String name) {
 * 
 * List<Benchmark> list = Collections.singletonList(new Benchmark()); if
 * (startPage!=null) { Pageable paging = PageRequest.of(0, 10,
 * Sort.by("submittedOn").descending()); Page page = new PageImpl(list);
 * when(benchmarkDao.findByUserId("test", paging)).thenReturn(page); } else
