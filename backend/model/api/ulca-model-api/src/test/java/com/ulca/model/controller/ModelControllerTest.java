...
class ModelControllerTest {
    ...
    void uploadModel() throws Exception {
        ...
        MockMultipartFile filePart = new MockMultipartFile("file", "test", null, fileContent);
        mockMvc.perform(multipart(BASE_URL+ "/upload").file(filePart)
                .param("userId", "test")
    ...
    void changeStatus() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post(BASE_URL+"/status/change").contentType(MediaType.APPLICATION_JSON)
                        .content(json)
                        .accept(MediaType.APPLICATION_JSON)).
                andExpect(MockMvcResultMatchers.status().isOk());
    }
    ...
    void computeModel() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post(BASE_URL+"/compute").contentType(MediaType.APPLICATION_JSON)
                        .content(json)
                        .accept(MediaType.APPLICATION_JSON)).
                andExpect(MockMvcResultMatchers.status().isOk());
    }
    ...
    void tryMeOcrImageContent() throws Exception {
        byte[] fileContent = "test".getBytes(StandardCharsets.UTF_8);
        MockMultipartFile filePart = new MockMultipartFile("file", "test", null, fileContent);
        mockMvc.perform(multipart(BASE_URL+ "/tryMe").file(filePart)
                        .param("modelId", "test").param("userId","test")
                        .contentType(MediaType.MULTIPART_FORM_DATA))
                .andExpect(MockMvcResultMatchers.status().isOk());
    }

    @Test
    void modelFeedbackSubmit() throws Exception {
        ModelFeedbackSubmitRequest request = new ModelFeedbackSubmitRequest();
        request.setTaskType("test");
        mapper = new ObjectMapper();
        String json = mapper.writeValueAsString(request);

        mockMvc.perform(MockMvcRequestBuilders.post(BASE_URL+"/feedback/submit").contentType(MediaType.APPLICATION_JSON)
                        .content(json)
                        .accept(MediaType.APPLICATION_JSON)).
                andExpect(MockMvcResultMatchers.status().isOk());
    }
    ...
    void getModelFeedbackByTaskType() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(BASE_URL+"/feedback/getByTaskType").param("taskType","test")).
                andExpect(MockMvcResultMatchers.status().isOk());
