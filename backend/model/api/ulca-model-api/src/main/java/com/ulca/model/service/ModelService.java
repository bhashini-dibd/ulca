...
//		List<ModelExtended> list = modelDao.findAll(example);
	...
	public ObjectNode getModelsPipeline(String jsonRequest, String userID, String ulcaApiKey) throws Exception {
		...
		TaskSpecifications individualTaskSpecifications = getIndividualTaskSpecifications(pipelineRequest.getPipelineTasks(),pipelineModel);
		log.info("INDIVIDUAL TASK SPECIFICATIONS :: "+individualTaskSpecifications.toString());

		//Generate Response Language List
		PipelineResponseLanguagesList pipelineResponseLanguagesList = getPipelineResponseLanguagesList(individualTaskSpecifications);
...
/*		
	...
	*/
    ...
    TaskSpecification getPossibleConfigForCurrentTask(LanguagePair lp, TaskSpecification modelTaskSpecification,
        ...
        if (previousTaskSpecifications.size() > 0) {
            if (lp.getSourceLanguage() != null) // There are configs in the task provided by user
            {
                ...
                for (ConfigSchema previousTaskSchema : previousTaskSpecifications
                        .get(previousTaskSpecifications.size() - 1).getTaskConfig()) {
            ...
            if (pipelineTask instanceof TranslationTask) {
                ...
                for (TaskSpecification pipelineSpecification : pipelineModel.getTaskSpecifications()) {
                    if (pipelineSpecification.getTaskType() == SupportedTasks.TRANSLATION) {
                        taskSpecifications
                                .add(getPossibleConfigForCurrentTask(lp, pipelineSpecification, taskSpecifications));
                    }
                }
