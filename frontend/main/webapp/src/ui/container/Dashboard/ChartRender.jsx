...
const ChartRender = (props) => {
  ...
  const handleLanguagePairChange = (value, property) => {
    ...
    if (sLang) {
      fetchChartData(selectedOption.value, "", [
        { field: "sourceLanguage", value: sLang.value },
      ]);
  ...
  const fetchFilterButtons = () => {
    return (
      <div>
        ...
        <Button
          color={filterValue === "primarySubmitterName" ? "primary" : "default"}
          size="small"
          variant="outlined"
          onClick={() => handleLanguageChange("primarySubmitterName")}
        >
          Submitter
        </Button>
      </div>
      ...
      <div>
        ...
        {filterValue !== "collectionMethod_collectionDescriptions" && (
          <Button
            style={{ marginRight: "10px" }}
            color={
              toggleValue === "collectionMethod_collectionDescriptions"
                ? "primary"
                : "default"
            }
            size="small"
        ...
        {filterValue !== "primarySubmitterName" && (
          <Button
            color={
              toggleValue === "primarySubmitterName" ? "primary" : "default"
            }
            size="small"
            variant="outlined"
            onClick={() => handleLevelChange("primarySubmitterName")}
          >
            Submitter
          </Button>
        )}
    ...
    switch (dataSet.value) {
      case "parallel-corpus":
        if (page === 0) {
          ...
          setTitle(
            `${
              sourceLanguage.label
            }-${selectedLanguageName} parallel sentences ${
      ...
      case "ocr-corpus":
        if (page === 0) {
          ...
          setTitle(
            `Number of images with ${selectedLanguageName} text ${
              filterValue === "primarySubmitterName" ? "uploaded by" : "of"
            } ${event.label ? event.label : dataValue}  - Grouped by ${
              filter === "domains"
                ? "Domain"
                : filter === "collectionMethod_collectionDescriptions"
