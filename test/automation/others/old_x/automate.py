...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--update-schema", help="updates the schema", action="store_true"
    )
    parser.add_argument("--chart-data", help="gets chart data", action="store_true")
    parser.add_argument(
        "-g",
        "--groupby",
        help="groupby in chart " + "['domain', 'collection', 'submitter']",
        type=str,
        default="",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-l", "--login", help="checks if credentials valid", action="store_true"
    )
    parser.add_argument(
        "-d", "--dataset", help="flag for dataset functions", action="store_true"
    )
    parser.add_argument(
        "-m", "--model", help="flag for model functions", action="store_true"
    )
    parser.add_argument(
        "-w", "--wrapper", help="complete automation flag", action="store_true"
    )
    ...
    parser.add_argument(
        "-src", "--source-lang", help="source language", type=str, default=""
    )
    parser.add_argument(
        "-tgt", "--target-lang", help="target language eg:hi,mr", type=str, default=""
    )
    parser.add_argument(
        "-dom", "--domain", help="domain for searched dataset", type=str, default=""
    )
    parser.add_argument(
        "-coll",
        "--collection-method",
    ...
    parser.add_argument(
        "-org", "--org-source", action="store_true", help="orginal source sentences"
    )
    parser.add_argument(
        "-b", "--benchmark", help="Benchamrk Name", type=str, default=""
    )
    ...
    # running test script
    if test_flag:
        driver = core.test_public_website(driver)
        driver = core.test_asr_live_record(driver)

    # login process
    if typex != "-corpus" and model_flag:
        login_status=True
        pass
    else:
    ...
    # dataset related processes
        ...
        if name != "":
            ...
            driver = dataset.perform_upload_with_csv(inp, driver)
        else:
            driver = dataset.perform_search_and_download(
                typex, tgt, src, domain, coll_mtd, m_anno, m_transl, org_source, driver
            )
    # model related processes
        ...
        if typex != "-corpus":
            ...
            if status and (benchmark != "" or metric != ""):
                status, driver = model.run_benchmark(name, benchmark, metric, driver)
                # if status:
                #    driver = model.check_benchmark_status(driver)
            if status and (unpublish_flag or publish_flag):
                driver = model.run_publish_task(
                    name, publish_flag, unpublish_flag, driver
                )
    # wrapper process
    elif wrapper_flag:
        print("wrapper method still in-progress.")
    else:
        core.print_no_flag_found(driver)

    # performs logount and closes browser-driver
    if login_status and typex == "-corpus":
